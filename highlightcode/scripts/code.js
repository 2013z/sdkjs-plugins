(function(window, undefined){
	...
	window.Asc.plugin.onTranslate = function()
	{
		...
		if (lb_lanuage)
			lb_lanuage.innerHTML = window.Asc.plugin.tr("语言");
		var btn_highlight = document.getElementById("btn_highlight");
		if (btn_highlight)
			btn_highlight.innerHTML = window.Asc.plugin.tr("高亮");
		var lb_style = document.getElementById("lb_style");
		if (lb_style)
			lb_style.innerHTML = window.Asc.plugin.tr("样式");
		var lb_repTab = document.getElementById("lb_repTab");
		if (lb_repTab)
			lb_repTab.innerHTML = window.Asc.plugin.tr("用空格缩进制表符");
		var opt_DontRep = document.getElementById("opt_DontRep");
		if (opt_DontRep)
			opt_DontRep.innerHTML = window.Asc.plugin.tr("不要缩进");
		var opt_2sp = document.getElementById("opt_2sp");
		if (opt_2sp)
			opt_2sp.innerHTML = window.Asc.plugin.tr("缩进 2 个空格");
		var opt_4sp = document.getElementById("opt_4sp");
		if (opt_4sp)
			opt_4sp.innerHTML = window.Asc.plugin.tr("缩进 4 个空格");
		var lb_bgColor = document.getElementById("lb_bgColor");
		if (lb_bgColor)
			lb_bgColor.innerHTML = window.Asc.plugin.tr("选择背景色");
		var btn_sp_cancel = document.getElementById("btn_sp_cancel");
		if (btn_sp_cancel)
			btn_sp_cancel.innerHTML = window.Asc.plugin.tr("取消");
		var btn_sp_choose = document.getElementById("btn_sp_choose");
		if (btn_sp_choose)
			btn_sp_choose.innerHTML = window.Asc.plugin.tr("选择");
