(function () {
    ...
    function displaySearchItems(append, own, res, err) {
        ...
        if (res && res.items.length > 0) {
            ...
            if (err) {
                ...
                notFound.textContent = own ? getMessage("在您的库中找不到任何内容") : getMessage("Nothing found");
    ...
    function formatInsertBibliography() {
        if (!selectedStyle) {
            showError(getMessage("未选择样式"));
            return;
        }
        if (!selectedLocale) {
            showError(getMessage("未选择语言"));
            return;
        }
    ...
    function insertInDocument(html) {
        if (html) {
            ...
            showError(getMessage("无法使用所选样式创建参考目录"));
        }
    };
